...
def run_work_container_no_mount(image_name: str,
    ...
    for key in cline_supported_keys:
        ...
        if value:
            api_keys[key] = value
    # Codex credentials are injected into the agent user's ~/.bashrc (CODEX_API_KEY)
    # or seeded as $CODEX_HOME/auth.json by codex_runner. `su - claude_user` resets
    # the environment, so any container-level env keys set here would not reach the
    # Codex process anyway -- auth is delivered via the login shell, not here.
    with semaphore:
